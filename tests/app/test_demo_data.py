...
def test_list_demo_images_returns_empty_when_dataset_not_downloaded(tmp_path, monkeypatch):
    monkeypatch.setattr(demo_data, "_IMG_DIR", str(tmp_path / "no_such_images"))
    monkeypatch.setattr(demo_data, "_CSV_PATH", str(tmp_path / "no_such.csv"))

    assert demo_data.list_demo_images() == []


def test_list_demo_images_reads_local_dataset(tmp_path, monkeypatch):
    img_dir = tmp_path / "train_images"
    img_dir.mkdir()
    csv_path = tmp_path / "train_1.csv"
