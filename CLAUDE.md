...
**Current phase:** Phase 8/9 done — PDF report generation (`src/report/`) + Streamlit dashboard (`src/app/`). Phase 6 is now done — the optic-disc/cup U-Net has been retrained on the pooled/re-split REFUGE2 data (2026-07-11): held-out test Dice `dice_rim=0.8937 dice_cup=0.8576 mean=0.8756`, up from the old domain-split checkpoint's `mean=0.5599` (see ROADMAP.md's Phase 6). Phase 7 (multi-disease) is in progress — glaucoma classifier code is built and smoke-tested but **not yet actually trained** (see ROADMAP.md's Phase 7 section for the exact resume command and full context — read that before touching Phase 7 code). AMD (ADAM dataset) and IDRiD DR cross-validation haven't been started.
