...
def predict(model: torch.nn.Module, image: np.ndarray, device: str = "cpu", mc_samples: int = 0) -> dict:
    ...
    `mc_samples > 0` additionally runs Monte-Carlo Dropout (see
    detection/mc_dropout.py) and adds "uncertainty_std" -- the 1-sigma spread of
    the predicted class's probability across that many stochastic passes. The
    deterministic point prediction above is left unchanged, so mc_samples=0
